...
FREQ_SWITCH_OFFSET = 1.0e6       # Hz - offset from center for each switched freq or try 0.5e6 difference
...
N_FFT             = 1024         # freq resolution ~ 586 Hz ~ 0.12 km/s -- try 1024 from 4096
NBLOCKS_ACC       = 1000         # SDR blocks per sub-integration call (~85 ms each) -- try 1000 from 50
SUB_WINDOW        = 10.0         # seconds to accumulate at each switched frequency
...
def _load_pointings():
    df = pd.read_csv("4_29_finals.csv")
    return list(zip(df["Longitude (l)"], df["Latitude (b)"]))
...
def accumulate_spectra(sdr0, sdr1, duration_sec, nblocks=NBLOCKS_ACC, n_fft=N_FFT):
    ...
    Combines I and Q channels into complex samples, mean-subtracts each
    block to remove DC offset, then computes normalized power spectra.
    ...
    while time.time() < t_end:
        if stop_event.is_set():
            break
        try:
            raw0 = sdr0.capture_data(n_fft, nblocks=nblocks)  # shape (nblocks, n_fft, 2)
            raw1 = sdr1.capture_data(n_fft, nblocks=nblocks)

            # Combine I and Q into complex, cast to float first to avoid int8 overflow
            iq0 = raw0[..., 0].astype(np.float32) + 1j * raw0[..., 1].astype(np.float32)
            iq1 = raw1[..., 0].astype(np.float32) + 1j * raw1[..., 1].astype(np.float32)

            # Mean-subtract each block independently to remove DC offset
            iq0 -= np.mean(iq0, axis=-1, keepdims=True)
            iq1 -= np.mean(iq1, axis=-1, keepdims=True)

            # FFT along sample axis, shift so DC is at center
            fft0 = np.fft.fftshift(np.fft.fft(iq0, n=n_fft, axis=-1), axes=-1)
            fft1 = np.fft.fftshift(np.fft.fft(iq1, n=n_fft, axis=-1), axes=-1)

            # Power spectrum, normalized by n_fft^2 to make amplitude independent of FFT length
            pwr0 = (np.abs(fft0) ** 2) / n_fft ** 2   # shape (nblocks, n_fft)
            pwr1 = (np.abs(fft1) ** 2) / n_fft ** 2

            # Average over blocks within this capture call
            s0 = np.mean(pwr0, axis=0)   # shape (n_fft,)
            s1 = np.mean(pwr1, axis=0)
